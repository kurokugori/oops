    ...
    <div class="panel panel-default" style="width:50%; margin:0 auto;">
        ...
        <form action="{{route('oopssave',['action'=>$action])}}" method = "post" enctype="multipart/form-data">
            ...
            <label>ID sản phẩm</label>
                <input type='text' class='form-control form-control-sm' name='id' value="{{$products->id??''}}" placeholder="Ví dụ: A001">
            <label>Mã danh mục sản phẩm</label>
                <input type='text' class='form-control form-control-sm' name='phone_brand_id' value="{{$products->phone_brand_id??''}}" placeholder="Ví dụ: 1">
            ...
            <label>Giá bán</label>
                ...
                @if($action == "edit")
                    <img src="{{ asset('storage/anh/'.$products->image_url) }}" width="200px" class='mb-1'>
